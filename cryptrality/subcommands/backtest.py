...
from cryptrality.core import schedule, State
...
def backtest(subparsers, module_name, argv):
    ...
    parser.add_argument('-o', '--out', dest='out',
                        help='Output folder', default='summary_reports')
    parser.add_argument('--stats', dest='stats',action='store_true',
                        help='Toggle on the report generation')
    parser.add_argument('--plots', dest='plots',action='store_true',
                        help='Toggle on the candlestick chart generation')
    parser.add_argument('--chart_window', dest='plot_freq',choices=['6h', '24h', '2d', '7d', '1M'],
                        help=('If plot is enabled, define the time window for the '
                            'candlestick data to be displayed'), default='6h')
    parser.add_argument('--exchange', dest='exchange', default='binance_spot',
                        help='Define the exchange to run the backtest', choices=supported_exchanges)
    parser.add_argument('--hold_asset', dest='hold_asset', default='BTC-USD',
    ...
    Runner = None
    #trading_modules = getmembers(exchanges, ismodule)
    trading_modules = [
    ...
    try:
        ...
        strategy_name = os. path. splitext(
            os.path.basename(args.strategy))[0]
    runner.run_forever()
    total_pnl = []
    pnl_str = ('%(symbol)s\t%(entry_date)s\t%(entry_price)s'
    ...
    out_trades_file = os.path.join(
        dir_out, 'all_trades_by_symbol.tsv')
    # for symbol in Runner.positions:
    #     for position in Runner.positions[symbol]:
    #         print(position)
    # for symbol in Runner.orders:
    #     for order in Runner.orders[symbol]:
    #         print(order)

    with open(out_trades_file, 'wt') as out_pnl:
            ...
            for position in Runner.positions[symbol]:
                ...
                if position.exit_price:
                    pos_data = {
                        ...
                        'entry_date': datetime.utcfromtimestamp(position.entry_time/1000),
                        'entry_price': position.price,
                        'exit_date': datetime.utcfromtimestamp(position.exit_time/1000),
                        'exit_price': position.exit_price,
                        'quantity': position.quantity, 'pnl': position.pnl}
                    out_pnl.write(pnl_str % pos_data)
    
    str_final = ('Total PnL: %(total_pnl)f\n'
    ...
    if args.plots:
        ...
        for symbol in Runner.plot_data:
            ...
            trade_charts(
                df.copy(), trades_i.copy(), h_name,
                symbol, Runner.plot_config,
                freq=args.plot_freq)
    ...
    if args.stats:
        ...
        positions.loc[:, 'entry_date'] = pd.to_datetime(positions['entry_date'])
        daily_out_html = os.path.join(dir_out, 'daily_performance.html')
        daily_out_tsv = os.path.join(dir_out, 'daily_performance.tsv')
        daily = positions.resample('d', on='entry_date').sum().dropna(how='all')
        daily['pnl'].to_csv(daily_out_tsv, sep='\t')
        qs.reports.html(
            INITIAL_BALANCE+daily["pnl"].cumsum(), args.hold_asset, output=daily_out_html,
            title=strategy_name)
        for symbol in symbols:
            ...
            daily_symbol = positions_symbol.resample('d', on='entry_date').sum().dropna(how='all')
            #positions_symbol.set_index('entry_date', inplace=True)
            qs.reports.html(
                (INITIAL_BALANCE / len(symbols)) + daily_symbol["pnl"].cumsum(), args.hold_asset, output=out_html_symbol,
