...
from cryptrality.misc import round_time, str_to_minutes, xopen, candle_close_timestamp
from cryptrality.core import RunnerClass, OrderSide, OrderType, OrderStatus, Order, Position
...
class Runner(RunnerClass):
    ...
    def setup_data(self, schedule, start, end, state):
        '''
        ...
        for param in schedule:
            period_info = {
                ...
                'interval_minutes': str_to_minutes(
                    param['interval']),
                'symbols': [],
        ...
        for period_info in self.schedule:
            ...
            for symbol in period_info['symbols']:
                klines_data_name = os.path.join(
                    CACHED_KLINES_PATH, '%s.csv.gz' % '_'.join(
                        ['binance_spot', symbol, period,
                         start.replace('-', '_'), end.replace('-', '_')]))
                if not os.path.exists(CACHED_KLINES_PATH):
                    os.makedirs(CACHED_KLINES_PATH)
                if os.path.exists(
                    klines_data_name) and os.path.isfile(
                        klines_data_name):
                    ...
                    print('Caching %s klines for %s' % (period, symbol))
                ...
                historical_data[period][symbol] = list(
                    (bc.list_to_klines(candle, symbol, period) for candle in start_data))
        ...
        if message_type == 'ACCOUNT_UPDATE':
            ...
            if len(positions_info) > 0:
                ...
                try:
                    ...
                    last_position = None
                if last_position and last_position.is_open and position_amount == 0:
                    ...
                    if last_position:
                        if last_position.is_closed:
                            ...
                            last_position.update(
                                position_amount, Runner.current_time, entry_price)
                    else:
                        add_position = True
                    
                    if add_position:
                        new_position = Position(
                            symbol, position_amount, entry_price, Runner.current_time)
                        new_position.open()
            ...
            if last_position is None:
                print("warning, create a new position obj")
            ...
            if updated_order is None:
                print('Warning, order %i is not recorder '
                    'in the system' % order_id)
            else:
                order_status = k['o']['X']
                # print('%s %s order %s was %s' % (
                #     event_time, symbol, order_id, order_status))
                new_status = OrderStatus(bc.order_status_msg_to_enum[order_status])
                ...
                last_position.orders[updated_order].filled_quantity = filled_quantity
                last_position.orders[updated_order].created_time = order_time
                last_position.orders[updated_order].executed_quantity = filled_quantity
                last_position.orders[updated_order].executed_price = order_price
                last_position.orders[updated_order].fees = filled_quantity * order_price * FEES
            ...
            try:
                ...
                symbol = k['ac']['s']
                #print('%s Update leverage to %iX for symbol %s' % (
                #    event_time, update_leverage, symbol))
            except KeyError:
                pass
            return
    
    def candle_data_handler(self, candle):
        '''
        ...
        if last_position is None or last_position.is_closed:
            return
        types_to_monitor = [
            OrderType.IfTouched, OrderType.Limit, OrderType.MakerLimit]
        status_to_monitor = [
            OrderStatus.Created, OrderStatus.PartiallyFilled, OrderStatus.Pending]
        ...
        for i in range(len(last_position.orders)):
            ...
            if order.type in types_to_monitor:
                if order.status not in status_to_monitor:
                    continue
                order_time = datetime.utcfromtimestamp(order.created_time / 1000)
                ...
                if check_iftouched and order.side == OrderSide.Buy:             
                    ...
                    order_new, order_filled = order_trade_update(
                        current_symbol, abs(quantity), side_str, order.limit_price,
                        candle['t'], close_position)
                        ...
                        if current_symbol in schedule[ 'symbols']:
                            try:
                                ...
                                Runner.historical_klines[
                                    current_interval][current_symbol] = self.update_ochl(
                                        Runner.historical_klines[
                                            current_interval][
                                                current_symbol], candle, schedule['window_size'])
                                ...
                                try:
                                    ...
                                    Runner.historical_klines[
                                        current_interval][current_symbol] = self.update_ochl(
                                            {}, candle, schedule['window_size'])
                            # schedule['fn'](state, Runner.historical_klines[
                            #     current_interval][current_symbol], current_symbol)
                ## check of execution
...
def config_update(symbol, leverage, timestamp):
    '''
...
def account_update(symbol, quantity, price):
    '''
...
def order_trade_update_msg(
    symbol, quantity, side, price, timestamp, status, order = {}, closing=False):
    '''
    ...
    if status == 'NEW':
        ...
        if closing:
            ...
            if position.quantity < 0:
                realized_profit = open_amount - close_amount - (
                        open_amount * FEES) - (close_amount * FEES)
            else:
                realized_profit = close_amount - open_amount  - (
                        open_amount * FEES) - (close_amount * FEES)
...
def order_trade_update(symbol, quantity, side, price, timestamp, closing=False):
    ...
    order_filled = order_trade_update_msg(
        symbol, quantity, side, price, timestamp, 'FILLED', deepcopy(order_new), closing)
...
def sync_klines(klines_periods):
    ...
    klines_list.sort(key=lambda x: datetime.utcfromtimestamp(x['E'] / 1000))
...
def order_market_amount(symbol, quantity, leverage=None):
    ...
    if quantity > 0:
        ...
        side_str = 'SELL'
    order_new, order_filled = order_trade_update(
        symbol, abs(quantity), side_str, Runner.current_price[symbol],
        Runner.current_time, close_position)
    if close_position:
        position_new = account_update(
            symbol, 0, 0)
    else:
    ...
    #     order.link_id = len(Runner.orders[symbol])
    #     Runner.orders[symbol].append(order)
    # except KeyError:
    #     Runner.orders[symbol] = []
    #     order.link_id = len(Runner.orders[symbol])
    #     Runner.orders[symbol].append(order)
    return order
...
def order_limit_amount(symbol, quantity, price, leverage=None, fallback=None):
    ...
    if quantity > 0:
        ...
        side_str = 'SELL'
    order_new, order_filled = order_trade_update(
        symbol, abs(quantity), side_str, stepped_price,
        Runner.current_time, close_position)
    if close_position:
        position_new = account_update(
            symbol, 0, 0)
        ...
        position_new = account_update(
            symbol, quantity, stepped_price)
    Runner.messages += [
        order_new, position_new, order_filled]
    order = Order(
        order_new['o']['i'], symbol, 1,
        order_side, quantity, stepped_price)
    order.created_time = Runner.current_time
    if close_position:
        order.close_position = True
    Runner.positions[symbol][-1].add_order(order)
...
def order_if_touched_amount(symbol, quantity, price_limit, leverage=None, trigger_side=None, trigger_with='market', fallback=None):
    '''
    ...
    #     'symbol': symbol,
    #     'idx': order.link_id}
    # Runner.orders_monitoring.append(order_check_dict)
    return order


def order_if_percent_touched_amount(symbol, quantity, price_limit_percent, leverage=None, trigger_side=None):
    current_price = Runner.current_price[symbol]
    price_limit = current_price + (current_price * price_limit_percent)
    order = order_if_touched_amount(symbol, quantity, price_limit, leverage, trigger_side)
    return order
...
def cancel_order(symbol, id):
    try:
        ...
        last_position = None
    if last_position is None or last_position.is_closed:
        print("There are no orders or positions yet for %s" % symbol)
        return
    ...
    try:
        ...
        cancel_idx = None
    if cancel_idx:
        print("cancel order %i" % cancel_idx)
