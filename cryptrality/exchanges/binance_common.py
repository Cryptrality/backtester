...
def list_to_klines(item: List[str], symbol: str, period_str: str) -> Dict[str, Union[int, str, Dict[str, Union[str, int, bool]]]]:
    """
    read the data from the CSV (split by ',') from historical
    data and return kline formatted as in the live data stream
    """
    period_delta = timedelta(minutes=str_to_minutes(period_str))
...
def get_step_size_futures(client: Client, symbol: str) -> Tuple[float, float]:
    """
    ...
    for s in futures_info["symbols"]:
        if s["symbol"] == symbol:
            for info_filter in s["filters"]:
                if info_filter["filterType"] == "LOT_SIZE":
                    step_size = float(info_filter["stepSize"])
                if info_filter["filterType"] == "PRICE_FILTER":
                    price_precision = float(info_filter["tickSize"])
